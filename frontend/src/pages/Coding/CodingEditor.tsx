...
import { useGetCodingProblems, useRunCodingProblem } from '@/services/api-hooks';

export function CodingEditor() {
  const [match, params] = useRoute('/coding/:topic/:level/:problemId');
  const problemId = params?.problemId || 'p1';
  
  const [code, setCode] = useState('// Write your solution here\n// Example for JS: parse input from lines array and print or return result\n');
  ...
  const { data: problemsData } = useGetCodingProblems({ topic, level }, { query: { retry: false } });
  const problems = problemsData?.problems || [];
  const problem = problems.find((p: any) => p.id === problemId) || problems[0] || null;

  useEffect(() => {
    console.debug('CodingEditor debug:', { problemId, topic, level, problems, problem });
  }, [problemId, topic, level, problems, problem]);

  const { mutateAsync: submitCode, isPending: isSubmitting } = useSubmitCodingProblem();
  const { mutateAsync: runCode, isPending: isRunning } = useRunCodingProblem();
  ...
  const handleRun = async () => {
    try {
      const res = await runCode({ data: { problemId, language, code } });
      // Show per-test results if available
      const msg = res.passed ? `Passed ${res.testsPassed}/${res.testsTotal} tests.` : (res.error || `Failed ${res.testsPassed}/${res.testsTotal} tests.`);
      setOutput({ passed: res.passed, msg, score: res.pointsEarned || 0 });
      // attach detailed results to console by overwriting output text
      if (res.testResults && res.testResults.length) {
        const lines = res.testResults.map((t: any, i: number) => `Test ${i+1}: ${t.passed ? 'OK' : 'FAIL'} | Expected: ${t.expected} | Actual: ${t.actual || ''}`);
        setOutput((o) => o ? { ...o, msg: [o.msg, ...lines].join('\n') } : { passed: res.passed, msg: lines.join('\n'), score: 0 });
      }
    } catch (err) {
      setOutput({ passed: false, msg: err?.message || 'Run failed', score: 0 });
    }
  };

  const handleSubmit = async () => {
    try {
      const res = await submitCode({ data: { problemId, language, code } });
      setOutput({
        passed: res.passed,
        msg: res.passed ? `Success! Passed ${res.testsPassed}/${res.testsTotal} tests.` : (res.error || `Failed ${res.testsPassed}/${res.testsTotal} tests.`),
        score: res.pointsEarned
      });
    } catch (err) {
      setOutput({ passed: false, msg: err?.message || 'Submit failed', score: 0 });
    }
  };
  ...
  useEffect(() => {
    if (!problem) return;
    // If problem provides starterCode, use it. Otherwise generate a simple template based on language.
...
`// ${problem.title}
// ${problem.description}
// Input format: ${problem.inputFormat || 'lines'}
function solution(lines) {
  // parse lines and implement
  return '';
}
`
      );
    ...
    <div className="h-[calc(100vh-4rem)] md:h-screen flex flex-col md:flex-row overflow-hidden bg-background">
      ...
      <div className="w-full md:w-1/2 lg:w-2/5 flex flex-col border-r border-border h-[40vh] md:h-full shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-background">
          <h1 className="text-2xl font-display font-bold mb-4">{problem ? ` ${problem.title}` : 'Problem'}</h1>
            ...
            {problem ? (
              <>
                ...
                {problem.sampleTestCases && (
                  <>
                    <h3 className="text-foreground font-bold mt-6 mb-2">Sample Test Cases:</h3>
                    {problem.sampleTestCases.map((t: any, idx: number) => (
                      <pre key={idx} className="bg-card p-4 rounded-xl border border-border text-sm font-mono text-foreground mb-4">Input: {t.input}\nOutput: {t.output}</pre>
                    ))}
                  </>
                )}
              </>
