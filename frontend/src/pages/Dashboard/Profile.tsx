...
import { getQuestSnapshot } from '@/features/analytics/quest';
...
export function Profile() {
  ...
  const localSnapshot = getQuestSnapshot();
  ...
  const stats = profile || {
    totalPoints: localSnapshot?.stats.totalPoints || 1250,
    aptitudeStreak: localSnapshot?.stats.currentStreak || 5,
    codingStreak: localSnapshot?.stats.codingActivity || 3,
    longestAptitudeStreak: localSnapshot?.stats.longestStreak || 12,
    longestCodingStreak: localSnapshot?.stats.longestStreak || 8,
    problemsAttempted: localSnapshot?.stats.testsCompleted || 45,
    problemsSolved: localSnapshot?.stats.testsCompleted || 32,
    accuracy: localSnapshot?.stats.strongestScore || 71,
    badges: ['First Blood', '7-Day Streak', 'Math Whiz'],
    heatmapData: localSnapshot?.charts.heatmap || []
  };
